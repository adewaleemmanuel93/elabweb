...
(function($){
	...
	$.fn.dsCountDown = function(givenOptions){
		...
		var defaults = {
			...
			elemSelWeeks: '',			// Leave blank to use default value or provide a string selector if the lement already exist, Example: .ds-ds.data.weeks
			...
			titleWeeks: 'Weeks',			// Set the title of ds.data.weeks
		...
		ds.init = function(){
			...
			if(! ds.options.elemSelWeeks){
				ds.prepend('<div class="ds-element ds-element-weeks">\
							<div class="ds-element-value ds-weeks">00</div>\
							<div class="ds-element-title">'+ ds.options.titleWeeks +'</div>\
						</div>');
				ds.data.elemWeeks = ds.find('.ds-weeks');
			}else{
				ds.data.elemWeeks = ds.find(ds.options.elemSelWeeks);
			}
					...
					ds.data.thread = setInterval(
						function(){
							...
							if(ds.data.left > 0){
								...
								if(ds.data.seconds <= 0 && (ds.data.minutes > 0 || ds.data.hours > 0 || ds.data.days > 0 || ds.data.weeks > 0)){
									ds.data.minutes --;
									ds.data.seconds = 60;
								}

								if(ds.data.minutes <= 0 && (ds.data.hours > 0 || ds.data.days > 0 || ds.data.weeks > 0)){
									ds.data.hours --;
									ds.data.minutes = 60;
								}

								if(ds.data.hours <= 0 && ds.data.days > 0 && ds.data.weeks > 0){
									ds.data.days --;
									ds.data.hours = 24;
								}

								if(ds.data.days <= 0 && ds.data.weeks > 0){
									ds.data.weeks --;
									ds.data.days = 7;
								}

								if(ds.data.elemWeeks)
									ds.data.elemWeeks.html((ds.data.weeks < 10 ? '0' + ds.data.weeks : ds.data.weeks));
								if(ds.data.elemDays)
