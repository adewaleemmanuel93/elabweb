        ...
        $.each(labVcMaps, function(mapIndex, mapEntry)
        {
            google.maps.event.addDomListener(window, 'load', function()
            {
                ...
                if(mapEntry.plusMinusZoom)
                {
                    ...
                    {
                        var $zooms = $("#" + mapEntry.id).parent().find('.mm-zoom');

                        $zooms.removeClass('hidden');

                        var controlUIzoomIn = $zooms.filter(".mm-zoom-in")[0],
                            controlUIzoomOut = $zooms.filter(".mm-zoom-out")[0];
                    ...
                    zoomControlDiv.setAttribute('className', 'mm-zoom-controllers');
